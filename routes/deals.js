...
exports.deleteDeal = function(req, res) {
	...
	simpleDB.deleteAttributes(params, function(err, data) {
		...
		if (err) {
			...
			res.status(500).send('<script type=\"text/javascript\"> alert( "Error deleting deal:" + err );window.location = \'document.referrer\'</script>');
		}
		else  {
			console.log("Record deleted successfully") ;
			console.log(data);           // successful response
			//res.status(200).send('{ "success": true, "msg": "Deal deleted successfully" }') ;
			res.status(200).send('<script type=\"text/javascript\"> alert("Deal deleted successfully"); window.location = \'document.referrer\'</script>');
		}
	});
